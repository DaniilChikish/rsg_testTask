...
namespace Utility
{
    ...
    public static class MathUtilities
    {
        ...
        public static string DecimalTo32Base(int decimalNumber)
        {
            string digits = "0123456789ABCDEFHIJKVWRTXYSMNPLQ"; // Digits for 32-based system
            int baseNumber = digits.Length; // Base number for conversion
            string thirtyTwoBasedNumber = string.Empty; // Resulting 32-based number
            ...
            {
                int remainder = decimalNumber % baseNumber; // Get remainder
                thirtyTwoBasedNumber = digits[remainder] + thirtyTwoBasedNumber; // Prepend digit to result
                decimalNumber /= baseNumber; // Update decimal number
            }

            return thirtyTwoBasedNumber;
        }
        static int Base32ToDecimal(string thirtyTwoBasedNumber)
        {
            string digits = "0123456789ABCDEFHIJKVWRTXYSMNPLQ"; // Digits for 32-based system
            int baseNumber = digits.Length; // Base number for conversion
            int decimalNumber = 0; // Resulting decimal number

            // Reverse the input 32-based number
            char[] charArray = thirtyTwoBasedNumber.ToCharArray();
            Array.Reverse(charArray);
            string reversedNumber = new string(charArray);

            for (int i = 0; i < reversedNumber.Length; i++)
            {
                char c = reversedNumber[i];
                int digitValue = digits.IndexOf(c); // Get digit value
                if (digitValue == -1)
                {
                    throw new ArgumentException($"Invalid character '{c}' in input 32-based number.");
                }

                decimalNumber += digitValue * (int)Math.Pow(baseNumber, i); // Add digit value to decimal number
            }
        ...
        /// <summary>
        /// Транспонування координат відносно двох опор
        /// </summary>
        /// <param name="firstAnchorPosition">Позиція першої опорної точки</param>
        /// <param name="secondAnchorPosition">Позиція другої опорної точки</param>
        /// <param name="firstAnchorP">Транспонована позиція першої точки</param>
        /// <param name="secondAnchorP">Транспонована позиція другої точки</param>
        /// <param name="tranponingPosition">Позція точки що транспонується</param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        public static Vector3 TranspoingCoordanate(Vector3 firstAnchorPosition, Vector3 secondAnchorPosition, Vector3 firstAnchorP, Vector3 secondAnchorP, Vector3 tranponingPosition)
        {
            float TransponCoordinate(float a, float b, float c, float a_, float b_)
            {
                return b - a == 0 ? 0 : (((b_ - a_) * (c - a)) / (b - a)) + a_;
            }
            return new Vector3(
                TransponCoordinate(firstAnchorPosition.x, secondAnchorPosition.x, tranponingPosition.x, firstAnchorP.x, secondAnchorP.x),
                TransponCoordinate(firstAnchorPosition.y, secondAnchorPosition.y, tranponingPosition.y, firstAnchorP.y, secondAnchorP.y),
                TransponCoordinate(firstAnchorPosition.z, secondAnchorPosition.z, tranponingPosition.z, firstAnchorP.z, secondAnchorP.z));
        }

        // Перевіряє, чи точка знаходиться всередині трикутників спрайта
        public static bool IsPointInsideSpriteMesh(Vector2 point, Vector2[] vertices, ushort[] triangles)
        {
            // Проходимо по кожному трикутнику в меші
            for (int i = 0; i < triangles.Length; i += 3)
            {
                ...
                // Використовуємо метод перевірки, чи точка знаходиться всередині трикутника
        ...
        // Функція для перевірки, чи точка знаходиться всередині трикутника
        public static bool IsPointInTriangle(Vector2 p, Vector2 v0, Vector2 v1, Vector2 v2)
        {
            // Використовуємо метод площин для перевірки всередині трикутника
            float dX = p.x - v2.x;
            float dY = p.y - v2.y;
        ...
        {
            // Визначаємо початкову точку часу Unix (1 січня 1970 року)
            DateTime unixEpoch = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);

            // Перетворюємо Unix timestamp в кількість тиков, додаючи кількість тиков для секунд
            long ticks = unixEpoch.Ticks + unixTimestamp * TimeSpan.TicksPerSecond;

            // Повертаємо DateTime, використовуючи отриману кількість тиков
            return new DateTime(ticks, DateTimeKind.Utc);
        }
        public static double DateTimeToUnixTimestamp(DateTime dateTime)
        {
            var unixStart = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
            return (dateTime.ToUniversalTime() - unixStart).TotalSeconds;
        }
        ...
        public static long TicksToUnixTimestamp(long ticks)
        {
            ...
            // Обчислюємо кількість тиков від Unix Epoch
            long unixTicks = ticks - unixEpoch.Ticks;

            // Перетворюємо тики в секунди (1 секунда = 10 млн тиков)
            return unixTicks / TimeSpan.TicksPerSecond;
        }
    }
}
